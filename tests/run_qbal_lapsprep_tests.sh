...
oracle_source="$repo_root/tests/verify_qbal_lapsprep.f90"
...
[[ -f "$verifier" && -f "$oracle_source" ]] || {
  printf 'missing reader verifier or Fortran oracle\n' >&2
  exit 2
}
...
lapsprep_sources=(
  ...
  "$repo_root/tests/qbal_lapsprep_stubs.f90"
  ...
  "$repo_root/src/common/cloud_bal_lapsprep_adapter.f90"
  "$repo_root/src/common/cloud_bal_pressure_analysis.f90"
  "$repo_root/src/common/cloud_bal_wps_adapter.f90"
  "$repo_root/src/lapsprep/module_setup.f90"
  "$repo_root/src/lapsprep/module_lapsprep_wps.f90"
  "$repo_root/src/lapsprep/lapsprep.f90"
)
lapsprep_objects=()
for source in "${lapsprep_sources[@]}"; do
  filename=${source##*/}
  lapsprep_objects+=("${filename%.f90}.o")
done
...
PY
  ...
  for source in "${lapsprep_sources[@]}"; do
    filename=${source##*/}
    compile_one "$source" "${filename%.f90}.o"
  done
  ...
  link_lapsprep() {
    ...
    local objects=("$build/$main_object")
    for object in "${lapsprep_objects[@]}"; do
      [[ "$object" == "$main_object" || "$object" == lapsprep.o ]] || \
        objects+=("$build/$object")
    done
    record_argv "link:$executable" "$CLOUD_BAL_FC" "${flags[@]}" \
      "${objects[@]}" "${nf_flibs[@]}" -o "$build/$executable"
  }
  link_lapsprep lapsprep.o lapsprep.exe
...
PY
  ...
  link_lapsprep lapsprep_k300_negative.o lapsprep_k300_negative.exe
  popd >/dev/null
}

hash_inputs() {
  local variant=$1 level_root=$2
  python3 - "$level_root/input_hashes.sha256" "$variant" "$repo_root" "$upstream" \
    ...
    "$cloud_bal_intlc" "$cloud_bal_setvars" "$oracle_source" \
    "${lapsprep_sources[@]}" <<'PY'
import hashlib
import sys
from pathlib import Path

output, variant, repo, upstream, writer_manifest, nf_config, netcdff, netcdf, nf_include, nc_include, compiler, imf, intlc, setvars, oracle_source, *lapsprep_sources = map(Path, sys.argv[1:])
paths = [
...
run_level() {
  ...
  hash_inputs "$variant" "$level_root"
