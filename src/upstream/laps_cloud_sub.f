...
cdis    Cloud-BAL status overlay: producer status and writer checks only.
cdis    Cloud analysis calculations and output metadata are unchanged.
...
! add by why(2022.1.19) start 
        integer c_i, c_j
        real    solar_transit_alt, low_alt, rrlat
! add by why(2022.1.19) end 
...
! 20190820 yhlee start
        REAL*4 sh_modelfg(NX_L,NY_L,KCLOUD)
        REAL*4 ref_modelfg(NX_L,NY_L,KCLOUD)
! 20190820 yhlee end
...
!       real*4 pres_3d(NX_L,NY_L,NZ_L)
        ...
        real closest_radar(NX_L,NY_L) !yhlee
...
!       Stuff for 2d fields
! 20190823 yhlee start
        real*4 ref_mt_modelfg(NX_L,NY_L) 
        real*4 rqc_2d(NX_L,NY_L)
! 20190823 yhhlee end
...
!frl060100
!cde read sfc td. It will be used in lgt routine to define LCL
        var = 'TD'
        ext = 'lsx'
        call get_laps_2d(i4time,ext,var,units,comment
     1                  ,NX_L,NY_L,td_sfc_k,istatus)
        ...
        if(istatus .ne. 1)then
            write(6,*)' Error reading SFC Dewpoint Temps'
            goto999
        endif
!cde -END
!frl060100
...
!frl060100
!cde    After tunning .. decided to be 1. (100km) [testing range 0.0001 - 1]
        weight_modelfg = 1.    ! Model used beyond ~100km from nearest obs
...
!add by why(2022.1.19) start 
!    .l_use_vis. true or false ... is dertermined by 
!    whether the solar_alt at a center(i/ni, j/nj) is less then low_alt
!    (./lib/ground_temp.f/function t_ground_k) 

      c_i = NX_L/2 
      c_j = NY_L/2

      print*,'W0> c_i, c_j', c_i, c_j 
      print*,'W1> lat, lon ', lat(c_i,c_j), lon(c_i,c_j), rlat 
      print*,'W2> ha, dec, alt',
     1       solar_ha(c_i,c_j),solar_dec,solar_alt(c_i,c_j)

      rrlat =  37.95382
      solar_transit_alt = 90. - abs(rrlat - solar_dec) 
      print*,'W3> rlat, solar_transit_alt ',rrlat, solar_transit_alt

      if (solar_ha(c_i,c_j) .lt. 0.) then ! morning 
          low_alt  = solar_transit_alt * .36
      else                     ! afternoon
          low_alt  = solar_transit_alt * .49 
      endif

      if(solar_alt(c_i,c_j) .lt. low_alt) then 
          l_use_vis =.false.
          print*,'WHY CHECK > l_use_vis option is turn off', 
     1           solar_alt(c_i,c_j), low_alt     
      endif 
!add by why(2022.01.19) end 
...
!frl20100526       call get_istat_39(i4time,t39_k,tb8_k,solar_alt,r_missing_data           ! I
       call get_istat_39(i4time,t39_k,tb8_k,solar_alt,r_missing_data     ! I
...
! 20190823 yhlee ends
!yhlee            if(lstat_radar_3dref_orig_a(i,j))then
...
!frl060100
!cde Insert lgt data
        call insert_lightning(i4time,clouds_3d,cld_hts
     1      ,NX_L,NY_L,NZ_L,KCLOUD
     1      ,t_sfc_k,td_sfc_k,pres_sfc_pa,topo
     1      ,istatus)                                            ! istat_lgt

        if(istatus .ne. 1)then
            write(6,*)
     1      'Bad status returned from insert_lightning'
        endif
!cde -END
!frl060100
...
102         format(1x,i3,2f10.1,i2)
     ...
     1                                       ,NX_L,NY_L,KCLOUD,istatus)
        ...
        if (istatus .ne. 1) then
            write(6,*) ' Error writing LC3 field'
            return
        endif
...
!                               not actually needed for anything.
        ...
        istatus = 0
        if (ni .le. 0 .or. nj .le. 0 .or. nk .le. 0 .or.
     1      nk .gt. NZ_CLOUD_MAX) return
