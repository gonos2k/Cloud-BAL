...
c     The remap arrays and lookup tables use the normalized 920-bin
c     dimensions.  Per-tilt metadata counts are retained in the NetCDF
c     common block so get_data_field masks padded bins before this buffer is
c     filled.  Short counts are valid; get_tilt_netcdf_data bounds them.
...
!     call lut_gen FORTRAN routine 
     ...
     1                ,firstGateRangeV*1000.,gateSizeV*1000.
     1                ,NX_L,NY_L,NZ_L,istatus)
      if(istatus .ne. 1)then
          write(6,*)' remap_sub: LUT generation failed'
          return
      endif
...
! call RADAR_INIT to assess the current tilt (first/last) and read next tilt
     ...
     1                                                ,istatus)
              if(istatus .ne. 1)then
                  write(6,*)' remap_sub: next tilt read failed',
     1                      i_tilt_proc_next
                  return
              endif
