...
c
      ...
      integer i_purge, i4time_kys, len_fname    !!!! kys
...
c     A missing or corrupt Nyquist value would make the velocity folding
c     expression below divide by zero or overflow.  Reject this tilt while
c     retaining the original FPE=0 diagnostic contract.
      IF (.not. (vel_nyquist .gt. 0. .and.
     :           vel_nyquist .lt. 1000.)) THEN
        write(6,*) ' Invalid Nyquist velocity; tilt rejected: ',
     :             i_tilt,vel_nyquist
        deallocate(Velocity)
        deallocate(Reflect)
        GO TO 998
...
c       call radar_qc(NX_L,NY_L,NZ_L,grid_rvel,istatus_qc)
  ...
  845     format(' REMAP_PROCESS > ',i4,' ref grids < ',i4
     :                                 ,'no data file written...')
          GO TO 998 ! fail closed: no product was written
        END IF
...
c
  ...
  890   format(a80)
     ...
     1                        ,istatus)       
            ...
            DO 550 j = 1,NY_L
            DO 550 i = 1,NX_L
              if(grid_ref(i,j,k).ge.100.) grid_ref(i,j,k)=-10.       !!!  kys
...
cyskim : get time end
 ...
 865            format(
     ...
     1             ,1x,a4,2x,a3,2x,a9)          
                ...
                call put_compressed_multi_3d(i4time_kys,ext,var_a  !!! kys
ckys            call put_compressed_multi_3d(i_product_i4time,ext,var_a       
...
ckys            call put_laps_multi_3d(i_product_i4time,ext,var_a
                call put_laps_multi_3d(i4time_kys,ext,var_a
     1                                ,units_a,comment_a,out_array_4d
     1                                ,NX_L,NY_L,NZ_L,nf,istatus)
            ...
            if(istatus .ne. 1)then
                write(6,*)' REMAP_PROCESS > output writer failed',
     1              istatus
            ...
            call put_remap_vrc(i4time_kys,comment_a(1)
ckys        call put_remap_vrc(i_product_i4time,comment_a(1)
     1             ,rlat_radar,rlon_radar,rheight_radar
     1             ,dgr
     1             ,grid_ref,NX_L,NY_L,NZ_L
     1             ,c3_radar_subdir,path_to_vrc,r_missing_data,istatus)
            if(istatus .ne. 1)then
                write(6,*)' REMAP_PROCESS > VRC writer failed',
     1              istatus
                GO TO 998
            endif
