! Cloud-BAL: first reader rejects malformed/bounds-unsafe input;
! missing or empty optional LSO remains status -1. Other routines unchanged.
c
c
...
c
	...
	integer*4 synum, awnum,menum,adnum,io_status
        logical file_exists
...
c
	call make_fnam_lp(i4time, filetime, istatus)
        if(istatus .ne. 1)goto998
	call get_directory('lso', infile, len)
	infile = infile(1:len) // filetime(1:9) // '.lso'
c
        inquire(file=infile,exist=file_exists,iostat=io_status)
        if(io_status .ne. 0)goto998
        if(.not. file_exists)goto999
	open(11,file=infile,status='old',err=998)
c
c.....	Read the header.
c
        read(11,'(a)',iostat=io_status) header_line
        if(io_status .ne. 0)goto998
        if(len_trim(header_line) .lt. 41)goto998
	read(header_line,900,iostat=io_status) btime,	! time
...
c
        if(n_obs_g .lt. 0 .or. n_obs_b .lt. 0 .or.
     &     n_obs_g .gt. n_obs_b .or. n_obs_b .gt. maxsta)goto998
        if(n_obs_b.eq.0) then
         print*, 'no obs in file ',infile,' returning'
         jstatus=-1
         close(11,iostat=io_status)
         return
        endif
           adnum = 0
           synum = 0
	...
	do k=1,n_obs_b
c
	   read(11,901,iostat=io_status)  stations(k),            !station id
...
c
     ...
     &                   p_ea(k), alt_ea(k)      !pressure exp accuracy, alt exp accuracy
        if(io_status .ne. 0)goto998
c
	  read(11,911,iostat=io_status)   vis(k), vis_ea(k),      !visibility, vis exp accuracy (miles)
     ...
     &                   sfcm(k), sfcm_ea(k)     !soil moist, soil moist temp exp accuracy (%)
        if(io_status .ne. 0)goto998
c
	  read(11,913,iostat=io_status)   pcp1(k),                !1-h precipitation (inches)
     ...
     &                   pcp_ea(k), snow_ea(k)   !precip and snow exp accuracy (inches)
        if(io_status .ne. 0)goto998
c
	  read(11,915,iostat=io_status)  kkk_s(k),                !num cld layers 
...
c
        if(kkk_s(k) .lt. 0 .or. kkk_s(k) .gt. 5)goto998
	  if(kkk_s(k) .gt. 0) then
	    do ii=1,kkk_s(k)
              read(11,917,iostat=io_status) store_cldamt(k,ii),
     &                                    store_cldht(k,ii)
        if(io_status .ne. 0)goto998
	    enddo !ii
	  endif
c
!KJH obs data sort
         if (reptype(k) .eq. 'SYNOP')then
          synum = synum + 1
         elseif (reptype(k) .eq. ' AWS ')then
          awnum = awnum + 1
         elseif (reptype(k) .eq. 'METAR')then
          menum = menum + 1
         elseif (reptype(k) .eq. 'MEDAS')then
          adnum = adnum + 1
         endif
!KJH obs data sort
	...
	enddo !k
        call make_fnam_lp(i4time, filetime, istatus)
        call get_directory('tmp', infile, len)
        infile = infile(1:len) // filetime(1:9) // '.tmp'
        open(333,file=infile,status='unknown',err=998)
        
        write(333,331,iostat=io_status) 'synop;',synum,'aws;',awnum,
     &     'metar;',menum,'amds;',adnum
        if(io_status .ne. 0)goto998
!331    format(a6,i3,3x,a4,i3,3x,a6,i3,3x,a5,i3)
 331    format(a6,i3,3x,a4,i3,3x,a6,i3,3x,a5,i5)
        close(333,iostat=io_status)
        if(io_status .ne. 0)goto998
c
!       endfile(11)
	close(11,iostat=io_status)
        if(io_status .ne. 0)goto998
c
c..... End of data reading.  Lets go home...
c
	jstatus = 1
	return
998     continue
        ...
        n_obs_b = 0
        jstatus = 0
        print *,'Invalid LSO input or summary output failure'
        return
999     continue
