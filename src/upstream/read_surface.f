...
c
	integer*4 i4time, wmoid(maxsta), jstatus, status
...
c
c     Bounded overlay: reject invalid names/times before reader calls.
        if(len .lt. 2) then
           print *,' ERROR. Invalid surface filename'
           istatus = 0
           return
        endif
        ...
        if(infile(len-1:len) .eq. 'qc')then
           if(len .lt. 16) then
              print *,' ERROR. Invalid LSOQC filename'
              istatus = 0
              return
           endif
           ...
           if(status .ne. 1) then
              print *,' ERROR. Invalid LSOQC time ',filetime
              istatus = 0
              return
           endif
...
c
           ...
           if(len .lt. 13) then
              print *,' ERROR. Invalid LSO filename'
              istatus = 0
              return
           endif
           ...
           call i4time_fname_lp(filetime,i4time,status)
           if(status .ne. 1) then
              print *,' ERROR. Invalid LSO time ',filetime
              istatus = 0
              return
           endif
...
c
           if(jstatus .ne. 1) then
              print *,' ERROR.  LSO reader returned status ',jstatus
