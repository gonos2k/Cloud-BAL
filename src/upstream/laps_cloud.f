...
cdis    Cloud-BAL status overlay: producer exit/status checks only.
...
!                               pass to laps_cloud.
        ...
        if (NX_L .le. 0 .or. NY_L .le. 0 .or. NZ_L .le. 0) then
           write (6,*) 'Invalid cloud analysis dimensions'
           go to 999
        endif

        if (N_PIREP .lt. 0 .or. maxstns .le. 0) then
           write (6,*) 'Invalid cloud observation dimensions'
           go to 999
        endif

        if (N_PIREP .gt. huge(maxstns)-maxstns) then
           write (6,*) 'Cloud observation dimensions overflow'
           go to 999
        endif
     ...
     1                  j_status)
        ...
        if (j_status(1) .eq. 1 .and. j_status(3) .eq. 1 .and.
     1      j_status(4) .eq. 1) then
           write(6,*) 'CLOUD_PRODUCER_SUCCESS'
           stop 0
        endif

999     continue
        write(6,*) 'CLOUD_PRODUCER_FAILED'
        stop 1
