...
!frl060100
!cde Add i4time to handle lightning..
!       subroutine get_cloud_deriv(ni,nj,nk,clouds_3d,cld_hts,       
        subroutine get_cloud_deriv(i4time,ni,nj,nk,clouds_3d,cld_hts,
...
!frl060100
!cde Add some variables to handle lightning
        character*150 filename,directory
        character*31 ext
        ...
        integer*4 lgt_read_status
        integer*4 lgt_source_missing
        integer*4 lgt_missing
        parameter (lgt_missing = -1)
        logical lgt_unit_open
!cde - End
!frl060100
...
!frl060100
!cde Construct file name for LGT file
        ext = lgt_ext
        ...
        n_lgt = lgt_missing
        lgt_unit_open = .false.
!       The lightning ingest writes its integer missing value per cell.
!       Preserve usable counts in the other cells of a partially covered grid.
        call get_i2_missing_data(lgt_source_missing,lgt_read_status)
        if(lgt_read_status .ne. 1)goto 999
        if(lgt_source_missing .ge. 0)goto 999
        open(20,file=filename,status='old',
     1       iostat=lgt_read_status,err=999)
        if(lgt_read_status .ne. 0)goto 999
        lgt_unit_open = .true.

        do i = 1, ni
        do j = 1, nj
          read(20,25,iostat=lgt_read_status,end=999,err=999)
     1         n_lgt(i,j)
          if(lgt_read_status .ne. 0)goto 999
          if(n_lgt(i,j) .eq. lgt_source_missing)then
            n_lgt(i,j) = lgt_missing
          else if(n_lgt(i,j) .lt. 0)then
            goto 999
          endif
        enddo
        enddo
...
!                    n_lgt_1d(k) = n_lgt(i,j) !!! for lightning number 
                    n_lgt_1d(k) = lgt_missing !!! for lightning number 
                enddo

                  
                  if( n_lgt(i,j) .ne. lgt_missing) then
...
! include lightning cloud type(test) `20.1.
                              if(n_lgt(i,j) .gt. 0) then
                                call get_cloudtype_lt(temp_1d(k_1d)
     1                           ,d_thetae_dz_1d(k_1d),n_lgt_1d(k_1d)
     1                           ,cld_base_m,cld_top_m,itype,c2_type)
                             else
...
!frl060100
!cde - Change threshold 45 -> 30 Since 2005/06/14 
!                               if(radar_3d(i,j,k_1d) .gt. 45.
                                if(radar_3d(i,j,k_1d) .gt. 30.
...
!  khlee test
            ...
            if(itype .eq. 11) then
             write(6,*)'ctype lightning',n_lgt_1d,cloud_type_1d, itype 
            endif
...
!frl060100
!cde Changed to consider lightning events..
! w/o cloud with lightning 

!                 call cloud_bogus_w   !!! original
!    1            (grid_spacing_cen_m,cloud_type_1d,heights_1d,nk,w_1d) !!! original
...
! khlee 2019.03.28. if lightning ! update `20.1
...
!!!!! for lightning(now it is used) `20.1 

        subroutine get_cloudtype_lt(temp_k,d_thetae_dz, n_lgt_k
     1                          ,cbase_m,ctop_m,itype,c2_type)

cdoc    Determine Cloud Type, given temperature and stability d(theta(e))/dz

        character*2 c2_type,c2_cloudtypes(11)

        data c2_cloudtypes
     1  /'St','Sc','Cu','Ns','Ac','As','Cs','Ci','Cc','Cb','Ct'/

        temp_c = temp_k - 273.15
        depth_m = ctop_m - cbase_m

! for lightning cloud type 
      if (n_lgt_k.gt.0 ) then
            itype = 11  ! Ct
...
!       Go from Stability to Cloud Type
     ...
     1              d_thetae_dz .ge. -.0005) then
        ...
        write(6,*) 'lightning cloud type c2_type=',c2_type
