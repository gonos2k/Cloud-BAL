...
cdis
cdis
c     Cloud-BAL: original-upstream status only; not research authority.
        program laps_deriv_main
        use cloud_bal_deriv_adapter, only: cloud_bal_deriv_entry
        ...
        call cloud_bal_deriv_entry(stage_enabled,istatus)
        if(istatus.ne.STATUS_OK)then
            write(6,*)'DERIV_STAGE_ADAPTER_FAILED'
            stop 1
        endif
        if(stage_enabled)stop
        ...
        if(NX_L.le.0 .or. NY_L.le.0 .or. NZ_L.le.0)go to 999
     ...
     1                  j_status)
        ...
        if(j_status(10) .ne. 1)go to 999
        write(6,*)'LCO_PRODUCER_SUCCESS'
        stop
999     write(6,*)'LCO_PRODUCER_FAILED'
        stop 1
        end
