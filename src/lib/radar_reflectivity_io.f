...
c       Cloud-BAL focused extraction from the legacy getradar.f translation
c       unit. Radial-velocity routines were intentionally omitted; the
c       reflectivity function bodies below are unchanged.
...
!yhlee Aug. 2019       Now a jacket routine without 'closest_radar' in the argument list
        ...
        real*4 closest_radar(imax,jmax)  ! yhlee
...
cdoc                            calls read_multiradar_3dref.
        ...
        real*4 closest_radar(imax,jmax)
...
!       2dref=0, 3dref=0 - missing data
        ...
        integer*4 istatus_2dref_a(imax,jmax)
        integer*4 istatus_3dref_a(imax,jmax)
!add by why (2021.12.9) start
        integer i4_tol
!add by why (2021.12.9) end
        ...
        closest_radar = r_missing_data ! yhlee
...
!           Read Reflectivity
            ...
            if(readext(1:3) .ne. 'vrz')then ! read vxx file
                !add i4_tol by why (2021.12.09) start
                i4_tol = 1200
                !add i4_tol by why (2021.12.09) end 
     ...
     1                             ,closest_vxx                      ! O
     1                             ,istatus)                         ! O
               closest_radar = closest_vxx ! initialize   !! yhlee
...
!               endif
     ...
     1                             ,rlat_radar,rlon_radar,rheight_radar)       
                               closest_radar(i,j) = closest_vxx(i,j) ! initialize !! yhlee
                            endif
...
!                          (useful for precip type, get_low_ref)
                ...
                if(l_low_fill .or. l_high_fill)then
                    ...
                    do j = 1,jmax
                     ...
                     closest_radar(i,j) = closest_vxx(i,j) !yhlee
...
!                               endif
                         ...
                         closest_radar(i,j) = closest_vrc(i,j) ! yhlee
