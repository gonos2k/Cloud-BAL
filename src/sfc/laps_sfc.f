...
c
c
! Cloud-BAL 2026-09-08: propagate required LSX failures as exit 1.
	program laps_sfc
c
...
!!++++ to ensure bk_vis 11-mile cap by WHY(2026.04.17.)____start
       do j =1, nj
           do i = 1, ni
               if (vis_bk(i,j). lt. 0.0) then
                   vis_bk(i,j) = 0.
                   !print*, 'WHY bg_vis check',i,j, 'vis_bk <0 )'
               elseif (vis_bk(i,j) .gt. 17703. ) then !! 11mile = 17703m
                   !print*, 'WHY bg_vis check',i,j, 'vis_bk > 11-mile)'
                   vis_bk(i,j) = 17703.
               endif
           enddo
       enddo
!!++++ to ensure bk_vis 11-mile cap by WHY(2026.04.17.)____end

	   call conv_m2miles(vis_bk,vis_bk,ni,nj)
	   call visg2log(vis_bk,ni,nj,badflag) ! conv miles to log(miles)
       print*,'WHY check' ,badflag
	   back_vis = 1
	else
	   print *,'     No background available for ',var_req
