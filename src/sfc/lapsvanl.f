...
!           td7(i,j) = make_td(700., t7_c, qgkg, 0.) + 273.15   ! in K
	    td7(i,j) = make_td(700., t7_c, qgkg, rt_ref) + 273.15   ! in K
!frl 2008.7.17 end
	   enddo !i
	   enddo !j
	endif
...
! ++ Before calling vlog2vis, 11-mile cap is adopted by  WHY(2026.04.17)__start
      do j = 1, jmax
       do i = 1, imax
        if (vis(i,j) .gt. 7.29) vis(i,j) = 7.29
       enddo
      enddo
! ++ Before calling vlog2vis, 11-mile cap is adopted by  WHY(2026.04.17)__end
...
c
! Cloud-BAL 2026-09-08: writer status is LSX authority.
        jstatus(3) = istatus
        if(jstatus(3) .ne. 1) return
