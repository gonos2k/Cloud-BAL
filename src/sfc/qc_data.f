...
c
	...
	integer istatus, jstatus, io_status
...
c       Cloud-BAL: distinguish optional absence from failed QC input.
        istatus = -2
	n_obs_curr = n_obs_b
!       missing = -99.9
        badflag = -99.9
...
c
	...
	open(60,file=outfile,status='unknown',iostat=io_status)
...
c
        istatus = 1
 999	write(60,*,iostat=io_status)
     &       ' ** qc of surface data complete. ** '
        if(io_status .ne. 0) istatus = -2
        close(60,iostat=io_status)
        if(io_status .ne. 0) istatus = -2
