...
!       Cloud-BAL 2026-09-08: explicit LT1 status and process failure.
!       PBL remains a separate optional diagnostic, not LT1 authority.
        ...
        if (istatus .ne. 1 .or. NX_L .le. 0 .or. NY_L .le. 0) then
           write (6,*) 'Error getting horizontal domain dimensions'
           go to 999
        endif
        call get_laps_dimensions(NZ_L,istatus)
        if (istatus .ne. 1 .or. NZ_L .le. 0) then
     ...
     1                       j_status)
        ...
        if(n_prods .ne. 1)go to 999
        if(iprod_number(1) .ne. 28261)go to 999
        if(j_status(1) .ne. 1)go to 999
        write(6,*)' LT1_PRODUCER_SUCCESS'
        stop

999     write(6,*)' LT1_PRODUCER_FAILED'
        stop 1
...
!  ******************** PBL SECTION ******************************************
        ...
        if(istatus .eq. 1)then
            j_status(1) = 1 ! LT1 written; PBL cannot change this.
...
!           pressure vertical grid.
     ...
     1                         ,NX_L,NY_L,NZ_L,i,j,pbl_top_m,istatus)
                ...
                if(istatus .ne. 1)then
                    write(6,*)' ERROR: PBL pressure-to-height failed'
                    go to 999
                endif
...
!           Write PBL file
     ...
     1                            ,2,istatus)
            if(istatus .ne. 1)then
                write(6,*)' ERROR: PBL write failed; LT1 unchanged'
            endif
