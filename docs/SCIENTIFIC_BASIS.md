...
현재 층 내 근사는 source의 \(u,v,r,\Delta z\)를 고정한 직선 segment다.
각 source의 연속 위치를 제한된 소단계로 이동시키고 마지막 도착점에서만
bilinear 분배한다. 중간 격자 재분배나 교차 source의 속도 평균은 하지 않는다.
따라서 소단계 수는 이 고정조건 해의 도착 분포를 바꾸지 않아야 한다.
층을 내려간 다음에는 새 source 조건으로 다시 계산하지만, 층 내부 shear·
상변화를 경로 적분한 모형이나 source 질량을 제거하는 시간수송은 아니다.
