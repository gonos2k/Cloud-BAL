...
2026-09-16 후속 검토에서는 역변환 출력이 기대값으로 미리 채워져 PHI 갱신 누락에도
시험이 통과하는 허점을 확인했다. 배경과 다른 후보의 해석해 및 재실행 가능한 scratch
음성시험으로 검출력을 보완하며, 상세 실행 근거와 잔여 범위는 MR 체크리스트에서 추적한다.
...
PR #6 병합 `3a7f469` 이후 보강은 같은 계획·체크리스트에서 추적한다. P0 부호·섭동과
P1 비균일격자 정확도를 분리하고 동일 donor의 omega 차이를 검사한다. 첫 native 구현은
새 private stage 재시도와 저장 candidate U/V 기반 하부 W 재계산을 선택했다.
MR-C4의 RHS 진단 상태와 MR-C5의 상변화·낙하·지표 유출·수치 조정 장부를 구분한다.
이는 설계 선택이며 구현·시험은 미완료다. 문서 변경만으로 기존 실행 증거를 폐기하거나
새 종료조건까지 PASS를 확대하지 않는다. 코드·입력·환경과 기존 검사 범위가 같은
근거는 유지하고, 새 조건은 해당 실제 경로의 추가 시험으로 검증한다.

PR #7 병합 `7320165` 이후에는 raw 원본과 준비 완료 seed를 구분하고, 하부 W·저장·
경계/halo의 마지막 수정 뒤 최종 질량·경계 잔차를 검사하도록 보강했다. W 전달 단독
영증분과 전체 pipeline 영변경을 구분하고, 물리 변경 후보에는 baseline 전체 동일성을
요구하지 않는다. 진단 on/off의 주 상태·지속 상태 불변도 기존 MR-C4/C5에 연결한다.
