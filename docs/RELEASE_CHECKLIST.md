...
| 강수 trajectory 입력 계약 | DONE | 공개 kernel 입구에서 config·shape·finite·범위·dp·domain·phase/species 일관성을 한 번 검사하고 출력/ledger도 재검사; 현재 index-space 수송은 균일 dx/dy만 허용하고 비균일 격자는 물리좌표 수송 구현 전 fail-closed |
...
| focused legacy QBAL의 background omega 필수성 | DONE | U/V/T/HT/SH와 함께 OM status도 필수이고 모든 above-ground pressure cell의 OM coverage를 검사; 누락 OM을 0으로 대체하여 balance를 계속하지 않음 |
