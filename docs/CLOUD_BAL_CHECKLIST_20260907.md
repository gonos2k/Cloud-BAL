...
기준 HEAD: `f837bad0bdf1050a34975d93b178b7dee4420094`  
...
- 전체 범위는 필수 본체 45개(G01–V06)와 별도 완료 항목 X01 1개다.
...
- 제공된 원문에는 XLSX workbook 자체가 없으므로 workbook을 읽었다고 주장하지 않는다.
...
| O05 | 구름·레이더 | 격자평균 dynamic target 및 R_w/식별가능성 | P0 | P5 | 부분구현 | 소스검토 | M06,M07,O03 | NOT_RUN | OPEN |
| O06 | 구름·레이더 | 완료된 Barnes 산출물의 Cloud-BAL LOS 연결 | P1 | P5/P6 | 부분구현 | 소스검토 | M06,O05 | NOT_RUN | OPEN |
| O07 | 구름·레이더 | 시간·storm-relative trajectory frame | P1 | P5 | 부분구현 | 소스검토 | M06 | NOT_RUN | OPEN |
| O08 | 구름·레이더 | 낙하속도의 mass-weighted / Z-weighted 분리 | P1 | P5 | 부분구현 | 소스검토 | O03,O06 | NOT_RUN | OPEN |
| T01 | 수분·열역학 | 수분 분석증분과 물리 수송 ledger 분리 | P0 | P3/P5 | 부분구현 | 소스검토 | M05,O03 | NOT_RUN | OPEN |
| T02 | 수분·열역학 | source-to-sink 종별 보존 수송 | P1 | P3/P5 | 부분구현 | 소스검토 | T01,O07,O08 | NOT_RUN | OPEN |
...
| V03 | 과학·운영 검증 | 0–6 h 강수·바람·구름·열역학 검증 | P0 | P7 | 구현미확인 | 소스검토 | E04,B06,V01 | NOT_RUN | OPEN |
| V04 | 과학·운영 검증 | 초기 음파·중력파와 cold-pool 안전성 | P0 | P7 | 부분구현 | 소스검토 | V03 | NOT_RUN | OPEN |
...
- 다음 작업: main `f837bad`의 exact-HEAD suite와 입력 snapshot receipt 재생성.
- 완료조건: 실행 SHA·tree·input/config/compiler hash와 결과가 동일 세대에 결속.
...
- 다음 작업: host microphysics 일치 forward operator와 retrieval parameter ID/R_Z 도입.
- 완료조건: phase/PSD/wavelength/bright-band 가정을 cell provenance와 불확실도에 반영.
...
### O05 — 격자평균 dynamic target 및 R_w/식별가능성

- 분류/증거: `부분구현` / `소스검토`; 긴급도 `P0`; 기존 단계 `P5`.
- 담당 역할: `미배정 — dynamic target·관측권한 책임`; 선행 ID: `M06,M07,O03`.
- 다음 작업: air-motion retrieval/분석풍/부력 근거·오차·영역평균을 한 계약으로 연결.
- 완료조건: core w를 grid mean으로 대입하지 않음; 독립 근거와 유한 오차 없으면 적용0.
- 적대/회귀시험: 0관측·poor geometry·반복관측·큰 R_w·상승 core 면적변화.
- 근거: [R03]. gate=`NOT_RUN`; tested_sha/evidence_uri/reviewer=`(빈 값)`; closure=`OPEN`.

### O06 — 완료된 Barnes 산출물의 Cloud-BAL LOS 연결

- 분류/증거: `부분구현` / `소스검토`; 긴급도 `P1`; 기존 단계 `P5/P6`.
- 담당 역할: `미배정 — Barnes 통합·LOS 계약 책임`; 선행 ID: `M06,O05`.
- 다음 작업: 별도 완료 release receipt를 가져와 frame/dealias/beam/uncertainty 검증.
- 완료조건: V_r=beam·(u,v,w−Vt_Z); assimilated/held-out 중복 제거.
- 적대/회귀시험: folded/dealiased provenance, Nyquist 필요조건, multi-radar rank, ID 중복.
...
- 분류/증거: `부분구현` / `소스검토`; 긴급도 `P1`; 기존 단계 `P5`.
- 담당 역할: `미배정 — terminal velocity·PSD 책임`; 선행 ID: `O03,O06`.
- 다음 작업: flux에는 Vt_mass, Doppler에는 Vt_Z 사용; PSD 없으면 근사/오차 표기.
- 완료조건: 하나의 terminal speed를 이유 없이 양쪽 관측·질량 연산에 공유하지 않음.
- 적대/회귀시험: 다분산 PSD known moments; updraft suspension와 flux 역산.
- 근거: [R11]. gate=`NOT_RUN`; tested_sha/evidence_uri/reviewer=`(빈 값)`; closure=`OPEN`.
...
### T02 — source-to-sink 종별 보존 수송

- 분류/증거: `부분구현` / `소스검토`; 긴급도 `P1`; 기존 단계 `P3/P5`.
- 담당 역할: `미배정 — 보존 수송 책임`; 선행 ID: `T01,O07,O08`.
- 다음 작업: finite-volume substep 또는 명시 steady reconstruction을 선택하고 주장 범위 고정.
- 완료조건: 진짜 시간수송이면 source 제거·sink 추가·boundary precipitation과 positivity 폐합.
- 적대/회귀시험: 순회순서/permutation·tilted shaft·surface sink·split step·CFL.
- 근거: [R03]. gate=`NOT_RUN`; tested_sha/evidence_uri/reviewer=`(빈 값)`; closure=`OPEN`.
...
- 다음 작업: loading 후보는 prior/tendency로 제한하고 pressure-gradient·entrainment·area를 함께 평가.
- 완료조건: dBZ→고정 하강 w 금지; 과포화 제거와 과냉각 방지; 관측 오차 내 응답.
- 적대/회귀시험: 같은 dBZ의 dry/moist subcloud·updraft·stratiform·cold-pool 사례.
- 근거: [R11]. gate=`NOT_RUN`; tested_sha/evidence_uri/reviewer=`(빈 값)`; closure=`OPEN`.
...
- 담당 역할: `미배정 — paired forecast·과학 평가 책임`; 선행 ID: `E04,B06,V01`.
- 다음 작업: 같은 model/physics/forcing으로 paired forecast 수행.
- 완료조건: FSS/CSI/POD/FAR/bias·vector wind RMSE·T/qv·cloud base/top·budget noninferiority.
- 적대/회귀시험: event-block bootstrap·regime worst-case·clear-sky false precipitation.
- 근거: [R03]. gate=`NOT_RUN`; tested_sha/evidence_uri/reviewer=`(빈 값)`; closure=`OPEN`.

### V04 — 초기 음파·중력파와 cold-pool 안전성

- 분류/증거: `부분구현` / `소스검토`; 긴급도 `P0`; 기존 단계 `P7`.
- 담당 역할: `미배정 — 초기화 파동·cold-pool 검증 책임`; 선행 ID: `V03`.
- 다음 작업: 초기 고빈도 ps tendency/divergence 진단; 모델 timestep 수준 sampling 별도.
- 완료조건: 분석잔차뿐 아니라 시간전진 고주파 excess가 대조군 허용범위 이내; 정당한 대류 유지.
- 적대/회귀시험: 10/30/60분+0–6h; sampling aliasing; DFI/IAU 없음/있음 대조.
- 근거: [R05]. gate=`NOT_RUN`; tested_sha/evidence_uri/reviewer=`(빈 값)`; closure=`OPEN`.
...
원문의 요구사항 ID·분류·선행조건은 유지한다. [설계 검토 §3](CLOUD_BAL_DESIGN_REVIEW_20260907.md)의 REV01–REV08은 별도 요구사항 분모가 아니라 해당 ID의 해석·시험 보완이다. 특히 B04의 PCG는 향후 설계이고 현행 recurrence는 가중 CR 형태다. O08의 `vt_z_mean`은 현재 mass-weighted proxy이며 Z-weighted 구현 완료가 아니다. M03/B01에는 이웃 pressure interval의 공통 면과 두께 평균을 구별하는 반례를 포함한다.
