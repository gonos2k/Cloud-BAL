...
|---|---|---|
| 동일 층 T/q 두께 기여 | PASS_SCOPED / MATCHED_LAYER_THERMODYNAMIC_ATTRIBUTION | PR41 공통 연산자·동일 지원층; HT 변경 아님 |
| 공통 기준면 상대 지오포텐셜 | PASS_SCOPED / ZERO_REFERENCE_RELATIVE_PRESSURE_FORCE | PR42 연속 지원층 누적, gap 비연결, 고립 기준면 처리 |
| 동일 pressure 삼각형 경도·동북 metric | PASS_SCOPED | PR42 실제 구면 chart와 중심 metric; 생산 adjoint와 별도 |
| 온도·수분 상대힘 벡터 합 | PASS_SCOPED | 동일 지원영역·선형 누적·경도; 벡터 크기 합과 구분 |
| 상태기저·표본행 분리 | PASS_SCOPED / MANUFACTURED_STATE_BASIS | 기존 fit·face 회귀 유지; 실제 하부 prior를 확보한 것은 아님 |
| PR41 assertion 집계 | CLOSED | 실제 계수 52회, 추가 수정 불필요 |
...
| PR #44 비교기 유한성·입력 링크 P2 | CLOSED_SCOPED | 지원영역 NaN/Inf·음의 bound·shape 오류 및 디렉터리/깨진 링크 거부; 실제 사례 재검증의 수치·지원영역 불변 |
...
물리 초기화의 완료 근거가 아니다.
MR-C0–MR-C6 전체 게이트에는 아직 실제 `PASS`가 없다. 위 현재 실행표의
`PASS_SCOPED`는 개별 연구·재현 범위의 폐합이며, 전체 물리 후보의 통과와 구분한다.
...
## 2026-09-17 PR #25 후속 실행 순서 — 당시 기록
...
### PR39 — cap 독립 profile 연구와 기존 유량 전환 진단

[수식·검증·실자료 근거](CAP_INDEPENDENT_PROFILE_20260922.md): cap이 바뀔 때
바람 재구성 자체가 교체되어 생기는 유한 점프를 기존 기압 prior와 분리했다.
고정 pressure 절점과 명시적 양의 분산을 사용하는 작은 Fortran 재구성은
하나의 profile을 먼저 정하고 cap을 적분 분할에만 쓴다. 실자료 분산은
확보되지 않아 임의 가중 재구성을 적용하지 않았다. 기존 연결의 cap은
12→13 UTC 1,177개, 13→14 UTC 1,094개 face에서 달라졌으며, 고정 입력
전환 한계도 별도로 기록했다. 이 값은 실제 시간 변화나 경계 forcing이 아니다.
기존 cap 기반 유량은 물리 profile 채택 전 P1 제한을 유지한다. 0–10 m
수송·상단·높이 기준·생산 ON/native 승인은 계속 OPEN이다.


### PR40 — 고정 endpoint profile과 공유면 적분

[모형·실행 근거](SHARED_PROFILE_FACE_20260922.md): 두 endpoint를 한 번씩
재구성한 뒤 면 내부를 선형 보간한다. 같은 고정 profile의 적분만 분할하며,
공통 지면 위 pressure 정의역 밖의 10 m 표본은 거부한다. 미지원 하부를
지하 절점이나 외삽으로 채우지 않는다. 실제 한 면의 명시적 시험 분산
민감도는 관측오차 보정이나 물리 경계 권한이 아니다. 0–10 m 수송,
전체 기주·영역 잔차, 비영 ON·native·예보 승인은 계속 OPEN이다.

13 UTC에서 pressure 정의역 425개 face, HT 지면 위·단조성까지 348개,
양 끝 10 m bracket까지 90개가 통과했다. 유량과 무관하게 선택한 첫 내부 면의
조건부 endpoint 재구성·공유 적분은 Intel O0/O2에서 통과했다. 같은 면의
0–10 m 평균 pressure span 113.2788 Pa는 계속 미정 유량이다.
병렬 scratch producer 재실행에서 동일 단계 HT와 최종 LT1을 원래 산출물과
일치시켰고, HT 생성 전 T/SH를 확보했다. 현재 verified launcher의 builder
해시가 오래되어 전체 source 폐합은 별도이며, 이 결과는 보존 바이너리의
조건부 재현이다. 최종 수분 변화의 두께 영향·절대 높이 기준은 계속 OPEN이다.


### PR41 — 동일 구간 온도·수분 두께 영향 분리

[두께 진단](THERMODYNAMIC_THICKNESS_ATTRIBUTION_20260922.md)은 PR40에서 확보한
HT 생성 직전 T/SH와 최종 LT1 T3/LQ3 SH를 같은 pressure·mask·적분법으로 비교한다.
1,228,494개 지면 위 정규 층에서 대칭 온도·수분 기여의 합이 직접 가온도 두께
변화와 Intel O0/O2 산술 범위에서 일치했다. 층별 총 변화는 −2.857763∼+3.142897 m,
각 기주의 유효 층 합은 −2.858205∼+9.601824 m이다. 지면 부분층을 완성한 값이나
관측 높이 오차가 아니다. 원래 HT는 그대로이며, producer의 상수·습도 근사·lookup과
이번 진단 연산자의 차이도 남아 있어 생성 단계 잔차를 최종 T/q 변화에 전부 돌리지 않는다.

[하부 정의역 계획](LOWER_PROFILE_DOMAIN_DESIGN_20260922.md)은 상태 절점·실제 표본·
하부 prior를 분리한다. LW3/LT1과 FUA/FUA를 임의로 섞거나 지하 유한값을 새 관측으로
쓰지 않는다. 실제 prior·오차·풍향·높이 기준을 정하기 전 전 영역 profile 확장은
`DESIGN_ONLY / NOT_AUTHORIZED`이다. 동일 단계 자료 부재와 두께 기여 미계산 상태는
이번 범위에서 갱신하지만, 절대 높이 기준·0–10 m 수송·완전한 기주 수지·공식 launcher·
비영 ON·native·예보는 계속 OPEN이다.


### PR42 — 공통 기준면 상대 지오포텐셜과 수평력

[상대 수평력 진단](RELATIVE_PRESSURE_FORCE_20260922.md)은 PR41의 같은 층 두께
변화를 공통 5,000 Pa 기준면까지 연속된 지원층에서만 누적한다. 기준면의 상대
지오포텐셜 변화는 진단상 0으로 명시하며, 실제 기준면의 미정 수평력은 별도다.
같은 pressure면 삼각형의 affine 경도를 실제 구면 등면적 metric으로 물리적인
동·북 가속도로 변환한다. 1,294,999개 node-level과 2,564,603개 triangle-level의
조건부 진단이 Intel O0/O2에서 통과했다. 1,000 hPa의 상대 총가속도 최대 크기
0.0200136 m/s²는 실제 전체 압력경도력·관측오차·바람 증분으로 승인하지 않는다.
HT/PS/바람을 수정하거나 지균풍으로 변환하지 않는다.

[고정 상태기저 실험](PROFILE_STATE_BASIS_20260922.md)은 기존 profile의 표본
연산자를 공개하고 원래 fit이 이를 재사용하도록 분리했다. 명시적인 제조 prior와
실제 표본 위치를 구분하는 수치시험이며, 실제 하부 prior·공분산·변경 권한은
제공하지 않는다. PR41 assertion 집계는 실제 계수 52회로 정정했다.
기준면·하부 정의역·0–10 m 유량·전체 기주 수지·공식 launcher·ON/native/예보의
기존 OPEN 상태는 유지한다.
