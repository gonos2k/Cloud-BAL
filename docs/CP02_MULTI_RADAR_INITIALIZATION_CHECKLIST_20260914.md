...
- O0/O2 각 23개 실제 compiler/link argv, build/runtime cwd, 실행 전후 입력 해시를
  기록했다. 616개 산출물 해시를 부모가 재확인했고 reader-state O0/O2 바이트가 일치했다.
  기존 `run_lapsprep_vapor.sh` 전체 회귀도 pinned O0/O2에서 exit 0이다.
- 유지 checkout 기준 최종 근거:
  `scratch/pr17_reader_20260916/Cloud-BAL/scratch/qbal_lapsprep_verified_20260916/manifest.json`,
  `scratch/pr17_reader_20260916/Cloud-BAL/scratch/pr17_regression_cwd/receipt.json`.
  이전 writer 또는 수정 중 소스로 수행한 탐색 실행·중단 실행은 이 최종 횟수에 포함하지 않는다.
